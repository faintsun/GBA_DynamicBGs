...
void moveMapUp();
void moveMapDown();
# 3 "Library/mapHandler.c" 2


# 1 "c:\\devkitarm\\arm-none-eabi\\include\\stdio.h" 1 3
# 29 "c:\\devkitarm\\arm-none-eabi\\include\\stdio.h" 3
...
# 30 "c:\\devkitarm\\arm-none-eabi\\include\\stdio.h" 2 3
...
# 43 "c:\\devkitarm\\arm-none-eabi\\include\\sys\\cdefs.h" 3
...
# 44 "c:\\devkitarm\\arm-none-eabi\\include\\sys\\cdefs.h" 2 3
...
# 36 "c:\\devkitarm\\arm-none-eabi\\include\\stdio.h" 2 3
# 1 "c:\\devkitarm\\lib\\gcc\\arm-none-eabi\\7.1.0\\include\\stddef.h" 1 3 4
...
# 60 "c:\\devkitarm\\arm-none-eabi\\include\\stdio.h" 3
...
# 61 "c:\\devkitarm\\arm-none-eabi\\include\\stdio.h" 2 3
...
# 7 "Library/mapHandler.c" 2



# 9 "Library/mapHandler.c"
CURRENTMAP area;
...
void loadMap(AREAMAP* m, int r, int c) {
 ...
 DMANow(3, area.WORLD_TILES, &((charblock *)0x6000000)[0], area.worldTileLen/2);
 for (int i = 0; i < 32; i++) {
  DMANow(3, &area.WORLD_MAP_BG2[((i + r)*(area.worldTileW)+(c))], &area.SCREEN_MAP_BG2[((i)*(32)+(0))], 32);
 }
...
void loadPalette(AREAMAP* m) {
 DMANow(3, (unsigned short*)m->pal, ((u16 *)0x5000000), 256);
}


void drawMap() {
 DMANow(3, area.SCREEN_MAP_BG2, &((screenblock *)0x6000000)[28], 1024);
}



void worldToScreen(int screenR, int screenC, int worldR, int worldC) {
 area.SCREEN_MAP_BG2[ ((screenR)*(32)+(screenC)) ] =
  area.WORLD_MAP_BG2[ ((worldR)*(area.worldTileW)+(worldC)) ];
}
...
void moveMapRight() {
 for (int x = 1; x >=0 ; x--) {
  ...
  int tempCursor = area.cursorC + (240 / 8);
  if (tempCursor > 32) tempCursor -= 32;
  if ( (area.cursorC + (240 / 8)) % 32 == 0 ) area.mapRight += 32;

  for (int i = 0; i < 32; i++) {
   if (i < area.cursorR) worldToScreen(i, tempCursor-1, area.tileR - (area.tileR - area.offsetR) + i + area.mapDown, area.tileC + (240 / 8) - 1);
    else worldToScreen(i, tempCursor-1, area.tileR - (area.tileR - area.offsetR) + i - area.mapUp, area.tileC + (240 / 8) - 1);
  }
 }
 drawMap();
}
...
void moveMapUp() {
 for (int x = 0; x <= 1; x++) {
  ...
  cursorReset();
  if ( (area.cursorR + (160 / 8)) % 32 == 0 ) area.mapDown -= 32;
...
void moveMapDown() {
 for (int x = 1; x >= 0; x--) {
  ...
  int tempCursor = area.cursorR + (160 / 8);
  if (tempCursor > 32) tempCursor -= 32;
  if ( (area.cursorR + (160 / 8)) % 32 == 0 ) area.mapDown += 32;

  for (int i = 0; i < 32; i++) {
   if (i < area.cursorC) worldToScreen(tempCursor-1, i, (160 / 8) + area.tileR-1, area.tileC - (area.tileC - area.offsetC) + i + area.mapRight);
   else worldToScreen(tempCursor-1, i, (160 / 8) + area.tileR-1, area.tileC - (area.tileC - area.offsetC) + i - area.mapLeft );
  }
 }
 drawMap();
}
